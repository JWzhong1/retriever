...
arguments
'''
logging_name = "contriever" + "7-10"
dataset_path = "data"
pretrain_tokenizer = "facebook/mcontriever-msmarco"
model_path_question = "model/ckpt/question_encoder"
model_path_reference = "model/ckpt/reference_encoder"
load_checkpoint = "model/ckpt/ckpt_contriever/step-869-epoch-0.ckpt"  # optional
save_checkpoint = "model/ckpt/ckpt_contriever/%s" % (time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time())))
max_epoch = 5
...
class QuestionReferenceDensity_fromContriever(torch.nn.Module):
    def __init__(self, model_path_question, model_path_reference) -> None:
        super().__init__()
        self.question_encoder = Contriever.from_pretrained(model_path_question)
        self.reference_encoder = Contriever.from_pretrained(model_path_reference)
...
train_set = load_dataset('json', data_files=dataset_path+"/train.jsonl", split='train')
test_set = load_dataset('json', data_files=dataset_path+"/test.jsonl", split='train')
...
model = QuestionReferenceDensity_fromContriever(model_path_question,model_path_reference)
